...
Sometimes, however, we **do** want the aesthetics of a graph to depend on a variable. 
...
- `base_size = `: update base font size
- `base_family = `: update based font family
- `base_line_size`: update base line size
...
`ggsave()` will export the *most recently run* ggplot graph by default, unless you give it the name of a saved ggplot object. 

So to export the faceted graph above at 180 dpi, width a width of 8" and a height of 7", we can use:
```{r}
ggsave(here::here("img","np_graph.jpg"), dpi = 180, width = 8, height = 7)
```
