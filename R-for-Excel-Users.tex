...
Let's try this on our fish\_counts data.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{fish_counts }
\end{Highlighting}
\end{Shaded}
...
##  7  2016 aque  blacksmith              1
##  8  2016 aque  garibaldi               1
##  9  2016 aque  rock wrasse             1
## 10  2016 aque  senorita               57
## # ... with 35 more rows
\end{verbatim}
...
\item
  Understand the similarities between dplyr::filter() + join() and Excel's \texttt{VLOOKUP}
\end{itemize}
...
In this session we'll use the \textbf{fish\_counts\_curated.csv} and \textbf{invert\_counts\_curated.xlsx} files.
...
Remember to always explore the data you've read in using functions like \texttt{View()}, \texttt{names()}, \texttt{summary()}, \texttt{head()} and \texttt{tail()}.
...
\hypertarget{example-filter-rows-by-matching-a-single-character-string}{%
\subsubsection{Example: filter rows by matching a single character string}\label{example-filter-rows-by-matching-a-single-character-string}}
...
\hypertarget{example-filter-rows-based-on-numeric-conditions}{%
\subsubsection{Example: filter rows based on numeric conditions}\label{example-filter-rows-based-on-numeric-conditions}}
...
\StringTok{  }\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{filter}\NormalTok{(tot_count }\OperatorTok{>=}\StringTok{ }\DecValTok{50}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\hypertarget{example-make-a-subset-of-the-fish_counts-df-that-contains-garibaldi-blacksmith-or-black-surfperch}{%
\subsubsection{\texorpdfstring{Example: Make a subset of the fish\_counts df that contains \emph{garibaldi}, \emph{blacksmith} OR \emph{black surfperch}}{Example: Make a subset of the fish\_counts df that contains garibaldi, blacksmith OR black surfperch}}\label{example-make-a-subset-of-the-fish_counts-df-that-contains-garibaldi-blacksmith-or-black-surfperch}}

There are several ways to write an ``OR'' statement, which will keep any observations that match Condition A \emph{or} Condition B \emph{or} Condition C. In this example, we will create a subset from \textbf{fish\_counts} that only contains rows where the \textbf{common\_name} is \emph{garibaldi} or \emph{blacksmith} or \emph{black surfperch}.

Use \texttt{\%in\%} to ask R to look for \emph{any matches} within a combined vector of strings:

\hypertarget{example-make-a-subset-of-the-fish_counts_curated-that-does-not-contain-observations-recorded-at-site-a}{%
\subsubsection{Example: Make a subset of the fish\_counts\_curated that does NOT contain observations recorded at Site A}\label{example-make-a-subset-of-the-fish_counts_curated-that-does-not-contain-observations-recorded-at-site-a}}

\hypertarget{example-conditionally-subset-with-values}{%
\subsubsection{Example: Conditionally subset with values}\label{example-conditionally-subset-with-values}}

\hypertarget{example-combo-w-string-matching-value-condition}{%
\subsubsection{Example: Combo w/ string matching \& value condition}\label{example-combo-w-string-matching-value-condition}}
