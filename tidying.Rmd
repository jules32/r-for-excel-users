...
The `janitor` package by Sam Firke is a brilliant collection of functions for some quick data cleaning. We recommend that you explore the different functions it contains. Here, we'll use `janitor::clean_names()` to convert all of our column headers to a more convenient case - the default is **lower_snake_case**, which means all spaces and symbols are replaced with an underscore (or a word describing the symbol), all characters are lowercase, and a few other nice adjustments. 
...
inverts_sep_all <- inverts_triple_unite %>% 
  separate(year_site_name, 
           into = c("the_year", "the_site", "the_name"), 
           extra = "merge")
```
...
We see that the resulting data frame has split *year_site_name* into three separate columns, *the_year*, *the_site*, and *the_name*, storing everything after the second break ("-") in *the_name* instead of dropping pieces. 
