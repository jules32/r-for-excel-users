...
So far we've been using a couple functions from base R, such as \texttt{seq()} and \texttt{date()}. But, one of the amazing things about R is that a vast user community is always creating new functions and packages that expand R's capabilities. In R, the fundamental unit of shareable code is the package. A package bundles together code, data, documentation, and tests, and is easy to share with others. They increase the power of R by improving existing base R functionalities, or by adding new ones.
...
\textbf{What's the difference between a package and a library?}\\
Sometimes there is a confusion between a package and a library, and you can find people calling ``libraries'' to packages.

Please don't get confused: \texttt{library()} is the command used to load a package, and it refers to the place where the package is contained, usually a folder on your computer, while a package is the collection of functions bundled conveniently.
...
A moment about \textbf{logical operators and expressions}. We can ask questions about the objects we made.
...
If this seems confusing, take comfort in the fact that you are already used to using \texttt{\#}s in a few ways in real life: it can mean ``number'' or ``pound'' or hashtags on social media.
...
Choose a name. Call it whatever you want (the shorter the better), or follow me for convenience. I will call mine \texttt{my-repo}.
...
\tightlist
\item
  Our working directory is set to \texttt{\textasciitilde{}/github/my-repo}. This means that I can start working with the files I have in here without setting the filepath. This is that when we cloned this from RStudio, it created an RStudio project, which you can tell because:
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{fish_counts <-}\StringTok{ }\KeywordTok{read_csv}\NormalTok{(here}\OperatorTok{::}\KeywordTok{here}\NormalTok{(}\StringTok{"data"}\NormalTok{, }\StringTok{"fish_counts_curated.csv"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{substrate_cover <-}\StringTok{ }\KeywordTok{read_excel}\NormalTok{(here}\OperatorTok{::}\KeywordTok{here}\NormalTok{(}\StringTok{"data"}\NormalTok{, }\StringTok{"substrate_cover_curated.xlsx"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{substrate_cover <-}\StringTok{ }\KeywordTok{read_excel}\NormalTok{(here}\OperatorTok{::}\KeywordTok{here}\NormalTok{(}\StringTok{"data"}\NormalTok{, }\StringTok{"curation/substrate_cover_curated.xlsx, skip = 4, na = "}\OperatorTok{-}\DecValTok{9999}\StringTok{")}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{carp_cover_}\DecValTok{2000}\NormalTok{ <-}\StringTok{ }\NormalTok{readxl}\OperatorTok{::}\KeywordTok{read_excel}\NormalTok{(here}\OperatorTok{::}\KeywordTok{here}\NormalTok{(}\StringTok{"data"}\NormalTok{, }\StringTok{"substrate_cover_curated.xlsx"}\NormalTok{, }\DataTypeTok{range =} \StringTok{"D12:I15"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{carp_cover_}\DecValTok{2000}\NormalTok{ <-}\StringTok{ }\NormalTok{readxl}\OperatorTok{::}\KeywordTok{read_excel}\NormalTok{(here}\OperatorTok{::}\KeywordTok{here}\NormalTok{(}\StringTok{"data"}\NormalTok{, }\StringTok{"substrate_cover_curated.xlsx"}\NormalTok{, }\DataTypeTok{range =} \StringTok{"D12:I15"}\NormalTok{, }\DataTypeTok{col_names =} \KeywordTok{c}\NormalTok{(}\StringTok{"site_name"}\NormalTok{, }\StringTok{"transect"}\NormalTok{, }\StringTok{"quad"}\NormalTok{, }\StringTok{"plot_side"}\NormalTok{, }\StringTok{"type"}\NormalTok{, }\StringTok{"coverage"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{kelp_counts <-}\StringTok{ }\NormalTok{readxl}\OperatorTok{::}\KeywordTok{read_excel}\NormalTok{(here}\OperatorTok{::}\KeywordTok{here}\NormalTok{(}\StringTok{"data"}\NormalTok{, }\StringTok{"kelp_counts_curated.xlsx"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{readxl}\OperatorTok{::}\KeywordTok{excel_sheets}\NormalTok{(here}\OperatorTok{::}\KeywordTok{here}\NormalTok{(}\StringTok{"data"}\NormalTok{, }\StringTok{"kelp_counts_curated.xlsx"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{kelp_golb <-}\StringTok{ }\NormalTok{readxl}\OperatorTok{::}\KeywordTok{read_excel}\NormalTok{(here}\OperatorTok{::}\KeywordTok{here}\NormalTok{(}\StringTok{"data"}\NormalTok{, }\StringTok{"kelp_counts_curated.xlsx"}\NormalTok{, }\DataTypeTok{sheet =} \StringTok{"golb"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{kelp_golb <-}\StringTok{ }\NormalTok{readxl}\OperatorTok{::}\KeywordTok{read_excel}\NormalTok{(here}\OperatorTok{::}\KeywordTok{here}\NormalTok{(}\StringTok{"data"}\NormalTok{, }\StringTok{"kelp_counts_curated.xlsx"}\NormalTok{, }\DataTypeTok{sheet =} \DecValTok{6}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{kelp_path <-}\StringTok{ }\NormalTok{here}\OperatorTok{::}\KeywordTok{here}\NormalTok{(}\StringTok{"data"}\NormalTok{, }\StringTok{"kelp_counts_curated.xlsx"}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{readr}\OperatorTok{::}\KeywordTok{write_csv}\NormalTok{(kelp_all_sites, here}\OperatorTok{::}\KeywordTok{here}\NormalTok{(}\StringTok{"data"}\NormalTok{, }\StringTok{"kelp_all_sites.csv"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\StringTok{  }\KeywordTok{write_csv}\NormalTok{(here}\OperatorTok{::}\KeywordTok{here}\NormalTok{(}\StringTok{"data"}\NormalTok{, }\StringTok{"kelp_ivee.csv"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{writexl}\OperatorTok{::}\KeywordTok{write_xlsx}\NormalTok{(kelp_all_sites, here}\OperatorTok{::}\KeywordTok{here}\NormalTok{(}\StringTok{"data"}\NormalTok{, }\StringTok{"kelp_all_sites.xlsx"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{inverts_july <-}\StringTok{ }\NormalTok{readxl}\OperatorTok{::}\KeywordTok{read_excel}\NormalTok{(here}\OperatorTok{::}\KeywordTok{here}\NormalTok{(}\StringTok{"data"}\NormalTok{, }\StringTok{"invert_counts_curated.xlsx"}\NormalTok{), }\DataTypeTok{range =} \StringTok{"B1:D56"}\NormalTok{)}
...
Within your existing version-controlled R project, create a new R Markdown document with title ``Session 4 - Data visualization with ggplot2.'' Remove everything below the first code chunk. Knit and save the .Rmd file within your project working directory as ``session\_4\_ggplot2''.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ca_np <-}\StringTok{ }\KeywordTok{read_csv}\NormalTok{(here}\OperatorTok{::}\KeywordTok{here}\NormalTok{(}\StringTok{"data"}\NormalTok{, }\StringTok{"ca_np.csv"}\NormalTok{))}
\NormalTok{ci_np <-}\StringTok{ }\KeywordTok{read_xlsx}\NormalTok{(here}\OperatorTok{::}\KeywordTok{here}\NormalTok{(}\StringTok{"data"}\NormalTok{, }\StringTok{"ci_np.xlsx"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{R-for-Excel-Users_files/figure-latex/unnamed-chunk-58-1.pdf}
...
Within those themes, we can also add graph-wide options for fonts and sizes. For example:

\begin{itemize}
\tightlist
\item
  \texttt{base\_size\ =}: update base font size
\item
  \texttt{base\_family\ =}: update based font family
\item
  \texttt{base\_line\_size}: update base line size
\end{itemize}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{ggsave}\NormalTok{(}\StringTok{"np_graph.jpg"}\NormalTok{, }\DataTypeTok{dpi =} \DecValTok{180}\NormalTok{, }\DataTypeTok{width =} \DecValTok{8}\NormalTok{, }\DataTypeTok{height =} \DecValTok{7}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Let's start a new RMarkdown file, I'll call mine \texttt{pivot\_lobsters.Rmd}.
...
\NormalTok{lobsters <-}\StringTok{ }\KeywordTok{read_csv}\NormalTok{(}\StringTok{"data/lobster_counts_curated.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
## 6  2012     8 8/20/12 ivee         3 C              60      0   300
\end{verbatim}
...
\hypertarget{lobster-pivot-table}{%
\subsubsection{lobster pivot table}\label{lobster-pivot-table}}
...
\includegraphics[width=0.8\linewidth]{img/pivot-table-menu}
...
\includegraphics[width=0.8\linewidth]{img/pivot-table-create}
...
\includegraphics[width=0.8\linewidth]{img/pivot-table-count-year}
...
Say it with me: ``pivot tables are group\_by and then summarize''. And just like pivot tables, you have flexibility with how you are going to summarize. For example, we can calculate an average, or a total. It looks like this (from \href{http://www.rstudio.com/wp-content/uploads/2015/02/data-wrangling-cheatsheet.pdf}{RStudio's cheatsheet}):
...
Let's try this on our \texttt{lobsters} data, just like we did in Excel. We will calculate the the total number of lobster by year. In R-speak, we will group\_by year and then summarize by counting using the \texttt{n()}, which is another function from \texttt{dplyr}. We'll also use the pipe operator \texttt{\%\textgreater{}\%}
...
\subsection{summarize multiple variables}\label{summarize-multiple-variables}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{year_summary <-}\StringTok{ }\NormalTok{lobsters }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{group_by}\NormalTok{(year) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{summarize}\NormalTok{(}\DataTypeTok{count =} \KeywordTok{n}\NormalTok{(), }
            \DataTypeTok{mean_size_mm =} \KeywordTok{mean}\NormalTok{(size_mm, }\DataTypeTok{na.rm =} \OtherTok{TRUE}\NormalTok{), }
            \DataTypeTok{sd_size_mm =} \KeywordTok{sd}\NormalTok{(size_mm, }\DataTypeTok{na.rm =} \OtherTok{TRUE}\NormalTok{))}
...
Summarize \texttt{lobsters} by site and assign it to a variable called \texttt{site\_summary}.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{site_summary <-}\StringTok{ }\NormalTok{lobsters }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{group_by}\NormalTok{(site) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{summarize}\NormalTok{(}\DataTypeTok{count =} \KeywordTok{n}\NormalTok{(), }
            \DataTypeTok{mean_size_mm =} \KeywordTok{mean}\NormalTok{(size_mm, }\DataTypeTok{na.rm=}\OtherTok{TRUE}\NormalTok{), }
            \DataTypeTok{sd_size_mm =} \KeywordTok{sd}\NormalTok{(size_mm, }\DataTypeTok{na.rm=}\OtherTok{TRUE}\NormalTok{))}

\NormalTok{site_summary}
\end{Highlighting}
\end{Shaded}
...
Great. It can be useful to summarize by both site and year, so that we can learn a little more about how things change over time across sites. And, awesomely, we are able to \texttt{group\_by} more than one variable. Let's do this together, and assign this to a new variable called \texttt{siteyear\_summary}:
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(DT) }\CommentTok{# install.packages("DT")}
\KeywordTok{library}\NormalTok{(knitr) }\CommentTok{# install.packages("knitr")}
\end{Highlighting}
\end{Shaded}
...
The \texttt{knitr} package has a function called \texttt{kable()} that will write the output of a dataframe as a table formatted in Markdown. Let's have a look:

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{kable}\NormalTok{(site_summary)}
\end{Highlighting}
\end{Shaded}
...
When we run this we see there are \texttt{\textbar{}} symbols separating columns, and there are \texttt{-\/-\/-}'s indicating the column headers. Additionally, the \texttt{:} on the right of each \texttt{-\/-\/-} symbol makes the values right-alighned.

Now let's knit our RMarkdown document and see what this looks like rendered. It's a nice, static table.

\hypertarget{dtdatatable}{%
\subsection{\texorpdfstring{\texttt{DT::datatable()}}{DT::datatable()}}\label{dtdatatable}}

The DT package had a function called \texttt{datatable}; let's try it with our \texttt{siteyear\_summary} object:

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{datatable}\NormalTok{(site_summary)}
\end{Highlighting}
\end{Shaded}
...
There are also many other options to walk through here: \url{https://rstudio.github.io/DT/}

\hypertarget{activity-4}{%
\subsection{Activity}\label{activity-4}}

Use \texttt{datatable} to create an interactive table for our \texttt{yearsite\_summary} object.
...
\begin{Highlighting}[]
\NormalTok{lobsters }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{millenia =} \DecValTok{2000}\NormalTok{,}
...
## # ... with 6,356 more rows, and 1 more variable: observer <chr>
\end{verbatim}
