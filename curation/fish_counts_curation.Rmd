...
# Try: senorita, blacksmith, black surfperch, rock wrasse, opaleye?

fish_subset <- fish_counts %>% 
  filter(common_name %in% c("senorita", "blacksmith", "black surfperch", "rock wrasse", "opaleye","garibaldi")) %>% 
  filter(year == 2018) %>% 
  drop_na(count) %>% 
  mutate(count = as.numeric(count)) %>% 
...
# Compiling counts by month and location for each fish type: 
