...
Let's try this on our fish_counts data. Let's summarize the total number of fish at each site. 

```{r}
fish_counts %>%
  group_by(site) %>%
  summarize(total_fish = sum(tot_count))
```

Awesome! And we can still `View(fish_counts)` and see that we haven't changed any of our original data. But maybe we do want to save this summary as a variable so we can refer to it or even include it in further analyses  
