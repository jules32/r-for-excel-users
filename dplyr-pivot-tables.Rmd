...
```{r, eval=FALSE, echo=FALSE, out.width="80%"}
knitr::include_graphics("img/pivot-table-sum.png")  
```
...
They are great because they summarize the data and keep the raw data raw — they even promote good pratice because they by default ask you if you'd like to present the data in a new sheet rather than in the same sheet.  
...
Let's try this on our `lobster_counts` data. Let's summarize this data by calculating the the total number of lobster by type, which in our dataframe is called `common_name`. We'll use the pipe operator `%>%`
...
This returns output summarizing the total_lobster for each lobster. There are way more senorita lobster than any other kind! 
...
```{r}
lobster_counts %>%
  summarize(total_lobster = sum(total_count))
```
...
Let's now check the `lobster_counts` variable. 
...
```{r}
common_name_summary <- lobster_counts %>%
