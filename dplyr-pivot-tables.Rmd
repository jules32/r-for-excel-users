...
lobster_counts %>%
  group_by(year) %>%
  summarize(# total_lobster = sum(count)
            mean, sd, n()) ) # within summarize, we name a new column and calculate the sum.


# if you only want counts and nothing else, count() is a shortcut for this. 
# group_by %>% summarize(n()) is the same as count()
```
