...
Let's say we need to a column with the type of observation this was, and all of them were by SCUBA. We use the mutate function, and inside it we will first name the column (`observation_type`) and set the value ("SCUBA"). We'll have to put SCUBA in quotes:
...
So to use mutate in combination with group_by. We are going to add a new column with total counts, calculated after grouping by site and year. Let's have a look at it first, and then we will assign it as a variable in a moment.
...
So in a case like this, hy would you use mutate instead of summarize? Why would you ever want to have this `total_fish` column with values repeated? Why wouldn't you always do group_by summarize rather than group_by mutate? Let's explore this: 
