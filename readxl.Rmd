...
- Use `read_excel()` to read in an Excel worksheet from a workbook
...
So far we've read in a single CSV file using `read_csv()`, and an Excel file containing a single worksheet with `read_excel()`. Now let's read in data from an Excel workbook with multiple worksheets. 
...
kelp_ivee <- read_excel("kelp_counts_curated.xlsx", sheet = "ivee") %>% 
  #select(year, total_fronds) %>%  TODO: change this to read in a range
...
- Did you know that Clippy shows up to help you in the documentation for ?write_xlsx()?
