...
In the .Rmd you just created, attach the `tidyverse`, `readxl`, `writexl`, and `here` packages  
...
fish_counts <- read_csv(here::here("data", "fish_counts_curated.csv"))
...
```{r, include = FALSE}
# REMOVED THIS (doing ggplot2 next chapter)
...
```{r, eval = FALSE, results = 'hide'}
substrate_cover <- read_excel(here::here("data", "substrate_cover_curated.xlsx"))
```
...
```{r, eval = FALSE}
substrate_cover <- read_excel(here::here("data", "curation/substrate_cover_curated.xlsx, skip = 4, na = "-9999")
```
...
```{r, eval = FALSE}
carp_cover_2000 <- readxl::read_excel(here::here("data", "substrate_cover_curated.xlsx", range = "D12:I15")
```
...
```{r, eval = FALSE}
carp_cover_2000 <- readxl::read_excel(here::here("data", "substrate_cover_curated.xlsx", range = "D12:I15", col_names = c("site_name", "transect", "quad", "plot_side", "type", "coverage"))
```
...
```{r, eval = FALSE, results = 'hide'}
kelp_counts <- readxl::read_excel(here::here("data", "kelp_counts_curated.xlsx")
```
...
```{r, eval = FALSE}
readxl::excel_sheets(here::here("data", "kelp_counts_curated.xlsx")
```
...
```{r, eval = FALSE, results = 'hide'}
kelp_golb <- readxl::read_excel(here::here("data", "kelp_counts_curated.xlsx", sheet = "golb"))
```
...
```{r, eval = FALSE}
kelp_golb <- readxl::read_excel(here::here("data", "kelp_counts_curated.xlsx", sheet = 6))
```
...
kelp_path <- here::here("data", "kelp_counts_curated.xlsx")
...
```{r, include = FALSE}
# REMOVED (since adding ggplot2 chapter AFTER this)
...
# Remove this (teaching complete in tidying section):
...
As a CSV: 
```{r, eval = FALSE}
readr::write_csv(kelp_all_sites, here::here("data", "kelp_all_sites.csv"))
```
...
kelp_ivee <- readxl::read_excel("kelp_counts_curated.xlsx", sheet = "ivee") %>% 
  ...
  write_csv(here::here("data", "kelp_ivee.csv"))
...
```{r, eval = FALSE}
writexl::write_xlsx(kelp_all_sites, here::here("data", "kelp_all_sites.xlsx"))
```
...
inverts_july <- readxl::read_excel(here::here("data", "invert_counts_curated.xlsx"), range = "B1:D56")
